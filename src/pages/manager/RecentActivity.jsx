...
const RecentActivity = () => {
    ...
    // Resolve which salon to scope to — URL param takes priority (impersonation mode)
    const querySalonId = searchParams.get('salonId');
    const effectiveSalonId = querySalonId || user?.salonId;

    // Show manager-style table whenever we have a specific salon scoped (direct login OR impersonation)
    const isManager = !!effectiveSalonId;

    useEffect(() => {
        let unsubs = [];

        if (effectiveSalonId) {
            // Scoped to a specific salon (manager login or super admin impersonating)
            unsubs = [
                subscribeToCollection(`salons/${effectiveSalonId}/sales`, setActivities),
                subscribeToCollection(`salons/${effectiveSalonId}/stylists`, setStylists),
                subscribeToCollection('salons', (data) => {
                    const mySalon = data.find(s => s.id === effectiveSalonId);
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                <div className="space-y-2">
                    ...
                    <p className="text-tea-400 text-[10px] font-black uppercase tracking-widest mt-1">
                        {isManager ? 'Live feed of your salon sales records' : 'Live feed of global salon operations and service sessions'}
                    </p>
                </div>
                            ...
                            <tr>
                                {isManager ? (
                                    <>
                                        ...
                                        <th className="text-left p-6 text-[10px] font-black text-tea-500 uppercase tracking-[0.2em]">Sold Products</th>
                                        ...
                                        <tr key={sale.id} className="hover:bg-tea-50/30 transition-colors group">
                                            ...
                                            <td className="p-6">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-xl bg-white border border-tea-100 shadow-sm overflow-hidden shrink-0">
                                                        <ImageWithFallback
                                            ...
                                            <td className="p-6">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <p className="text-sm font-black text-tea-700 uppercase tracking-tight">
                                                        {sale.clientName || 'General Client'}
                                                    </p>
                                                </div>
                                            </td>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        <p className="text-xs font-black text-tea-900 uppercase tracking-tighter">
                                                            {new Date(saleDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                                        </p>
                                                        <p className="text-[9px] font-black text-tea-400 uppercase tracking-widest">
                                                            {new Date(saleDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                        </p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="p-6 text-right">
                                                <div className="space-y-1">
                                                    <p className="text-xl font-black text-tea-900 tracking-tighter leading-none">${amount}</p>
                                    ...
                                    <tr key={sale.id} className="hover:bg-tea-50/30 transition-colors group">
                                        ...
                                        <td className="p-6">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                    <p className="text-xs font-black text-tea-900 uppercase tracking-tighter">
                                                        {new Date(saleDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                                                    </p>
                                                    <p className="text-[9px] font-black text-tea-400 uppercase tracking-widest">
                                                        {new Date(saleDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                    </p>
